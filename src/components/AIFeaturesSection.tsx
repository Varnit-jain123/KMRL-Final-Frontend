...
export const AIFeaturesSection = ({ onDocumentClick }: AIFeaturesSectionProps) => {
  ...
  // Mock AI features data with translation keys
  const aiSummariesData = [
    {
      id: "summary1",
      documentTitleKey: "ai.document.q3.report",
      originalSize: "2.4 MB",
      summaryLengthKey: "ai.file.words",
      summaryLengthCount: "250",
      originalLengthKey: "ai.file.pages", 
      originalLengthCount: "45",
      confidence: 94,
      summaryKey: "ai.summary.q3.content",
      keyPoints: [
        "ai.keypoints.revenue.growth", 
        "ai.keypoints.cost.reduction", 
        "ai.keypoints.projects.completed"
      ],
      audioUrl: null,
      isPlaying: false
    },
    {
      id: "summary2", 
      documentTitleKey: "ai.document.metro.timeline",
      originalSize: "1.8 MB",
      summaryLengthKey: "ai.file.words",
      summaryLengthCount: "180",
      originalLengthKey: "ai.file.pages",
      originalLengthCount: "32",
      confidence: 89,
      summaryKey: "ai.summary.metro.content",
      keyPoints: [
        "ai.keypoints.project.completion", 
        "ai.keypoints.on.schedule", 
        "ai.keypoints.stations.operational"
      ],
      audioUrl: null,
      isPlaying: false
    }
  ...
  const aiTagsData = [
    { 
      documentKey: "ai.document.budget.allocation", 
      tags: ["ai.tags.finance", "ai.tags.budget", "Q4", "ai.tags.allocation"], 
      confidence: 96 
    },
    { 
      documentKey: "ai.document.safety.guidelines", 
      tags: ["ai.tags.safety", "ai.tags.protocol", "ai.tags.guidelines", "ai.tags.training"], 
      confidence: 92 
    },
    { 
      documentKey: "ai.document.training.manual", 
      tags: ["HR", "ai.tags.training", "ai.tags.onboarding", "ai.tags.procedures"], 
      confidence: 88 
    }
  ];
  ...
  const aiRemindersData = [
    { 
      id: "ai1", 
      titleKey: "ai.reminders.budget.review", 
      documentKey: "ai.document.budget.allocation", 
      dueDate: "2024-01-25", 
      confidence: 87 
    },
    { 
      id: "ai2", 
      titleKey: "ai.reminders.safety.training", 
      documentKey: "ai.document.safety.guidelines", 
      dueDate: "2024-01-30", 
      confidence: 91 
    },
    { 
      id: "ai3", 
      titleKey: "ai.reminders.project.milestone", 
      documentKey: "ai.document.metro.timeline", 
      dueDate: "2024-02-05", 
      confidence: 84 
    }
  ];

  const [summaries, setSummaries] = useState(aiSummariesData);
  const [autoSummariesEnabled, setAutoSummariesEnabled] = useState(true);
  const [autoTaggingEnabled, setAutoTaggingEnabled] = useState(true);
  const [activeTab, setActiveTab] = useState("summaries");
  ...
  const handleAIFeature = (feature: string, itemId: string) => {
    switch (feature) {
      case 'summary':
        // Generate a mock AI summary using translation
        const summaryText = t('ai.summary.generated.content') || 
        ...
        toast({
          title: t('ai.summary.generated'),
          description: t('ai.summary.generated.desc'),
        });
        break;
        
      case 'tag':
        toast({
          title: t('ai.tagging.analyzing'),
          description: t('ai.tagging.analyzing.desc'),
        });
        setTimeout(() => {
          toast({
            title: t('ai.tagging.generated'),
            description: t('ai.tagging.generated.desc'),
          });
        }, 1500);
        break;
      ...
      case 'podcast-play':
        toast({
          title: t('ai.podcast.playing'),
          description: t('ai.podcast.playing.desc'),
        });
        break;
        
      default:
        toast({
          title: t('ai.tagging.activated'),
          description: t('ai.tagging.activated.desc').replace('{feature}', feature),
        });
    }
  };
  ...
  const handleCopySummary = () => {
    if (showSummary) {
      navigator.clipboard.writeText(showSummary.text);
      toast({
        title: t('ai.summaries.copied'),
        description: t('ai.summaries.copied.desc'),
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center space-x-2">
          <Brain className="h-8 w-8 text-primary" />
          <h2 className="text-3xl font-bold">{t('ai.features.title')}</h2>
        </div>
        <p className="text-muted-foreground max-w-2xl mx-auto">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="summaries" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {summaries.map((summary) => (
              <Card key={summary.id} className="hover:shadow-sm transition-all">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-medium">{t(summary.documentTitleKey)}</h3>
                    <Badge variant="outline" className="text-xs">{summary.confidence}%</Badge>
                  </div>
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    {t('ai.summaries.last.processed')}: {summary.originalLengthCount} {t(summary.originalLengthKey)}
                  </p>
                  <Button size="sm" className="w-full" onClick={() => handleAIFeature('summary', summary.id)}>
                    <Brain className="h-3 w-3 mr-1" />
                    {t('ai.summaries.generate')}
                  </Button>
                </CardContent>
              </Card>
            ))}
          ...
          <Card className="bg-muted/5">
            <CardContent className="p-4">
              <h4 className="font-medium mb-2">{t('ai.summaries.tips.title')}</h4>
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>{t('ai.summaries.tips.auto')}</li>
                <li>{t('ai.summaries.tips.nlp')}</li>
                <li>{t('ai.summaries.tips.confidence')}</li>
              </ul>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="podcast" className="space-y-4">
            ...
            {summaries.map((summary) => (
              <Card key={summary.id} className="hover:shadow-sm transition-all">
                <CardContent className="p-4">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    {t('ai.podcast.available')}
                  </p>
                  <p className="text-xs text-muted-foreground mb-3">
                    {t('ai.podcast.last.generated')}: {summary.summaryLengthCount} {t(summary.summaryLengthKey)}
                  </p>
                  <div className="flex space-x-2">
                    <Button size="sm" onClick={() => handlePlayAudio(summary.id)}>
                      {summary.isPlaying ? (
          ...
          <Card className="bg-muted/5">
            <CardContent className="p-4">
              <h4 className="font-medium mb-2">{t('ai.podcast.features.title')}</h4>
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>{t('ai.podcast.features.voice')}</li>
                <li>{t('ai.podcast.features.speed')}</li>
                <li>{t('ai.podcast.features.offline')}</li>
              </ul>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="tagging" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {aiTagsData.map((item, index) => (
              <Card key={index} className="hover:shadow-sm transition-all">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-medium">{t(item.documentKey)}</h3>
                    <Badge variant="outline" className="text-xs">{item.confidence}%</Badge>
                  </div>
                  <div className="flex flex-wrap gap-1 mb-3">
                    {item.tags.slice(0, 3).map((tag, tagIndex) => (
                      <Badge key={tagIndex} variant="secondary" className="text-xs">
                        {tag.startsWith('ai.tags.') ? t(tag) : tag}
                      </Badge>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground mb-3">
                    {t('ai.tagging.last.processed')}
                  </p>
                  <Button size="sm" className="w-full" onClick={() => handleAIFeature('tag', item.documentKey)}>
                    <Tag className="h-3 w-3 mr-1" />
                    {t('ai.tagging.auto.tag')}
                  </Button>
                </CardContent>
              </Card>
            ))}
          ...
          <Card className="bg-muted/5">
            <CardContent className="p-4">
              <h4 className="font-medium mb-2">{t('ai.tagging.instructions.title')}</h4>
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>{t('ai.tagging.instructions.detect')}</li>
                <li>{t('ai.tagging.instructions.manual')}</li>
                <li>{t('ai.tagging.instructions.confidence')}</li>
              </ul>
            </CardContent>
          </Card>
        </TabsContent>
